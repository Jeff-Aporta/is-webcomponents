...
const css = fs.readFileSync(path.join(root, 'styles/presentation.css'), 'utf8');
const component = fs.readFileSync(path.join(root, 'components/actions/button.js'), 'utf8');

const system = fs.readFileSync(path.join(root, 'styles/is-base.css'), 'utf8');
const styles = `${system}\n${css}`;
...
assert(/id="fullscreenBtn"[\s\S]*?appearance="plain"[\s\S]*?pill/.test(html) || /appearance="plain"[\s\S]*?pill[\s\S]*?id="fullscreenBtn"/.test(html), 'fullscreen must be plain+pill icon button');
