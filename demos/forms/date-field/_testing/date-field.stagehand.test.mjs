// date-field.stagehand.test.mjs — rubric visual determinista para date-field.
//
// Sin LLM: chequeamos con Playwright + atributos del shadow DOM que el campo
// cumple cinco invariantes visuales:
//   1. Hay una sección por componente de fecha (3 spinbutton).
...
const VISUAL_RUBRIC = `
Evalúa la calidad visual del campo de fecha que aparece en el screenshot.

Checklist (todas deben cumplirse; marca cada una PASS o FAIL):

1. SECCIONES LEGIBLES: el campo de fecha muestra tres secciones (día, mes, año) claramente separadas por separadores. Cada sección es legible.
...
async function checkDeterministic(page) {
  ...
  for (const f of data) {
    ...
    // (1) Hay 3 secciones.
    assert.equal(f.sections.length, 3, `${tag}: debe haber 3 spinbutton (y/m/d)`);
    ...
    for (let i = 0; i < f.sections.length; i++) {
      ...
      assert.ok(s.w >= 14, `${tag}: sección ${i} muy estrecha (${s.w.toFixed(1)}px < 14px)`);
    }
...
try {
  ...
  console.log('  ✓ date-field: rubric determinista PASS');
  results.push({ name: 'date-field', skipped: false });
} catch (err) {
  console.error(`  ✗ date-field: ${String(err?.message ?? err)}`);
  try { await screenshot(page, 'fail-date-field'); } catch {}
  results.push({ name: 'date-field', error: String(err?.message ?? err) });
} finally {
...
if (process.env.STAGEHAND === '1') {
  ...
  try {
    ...
    if (!sh) {
      console.log('  ⊘ date-field: stagehand LLM sin credenciales, skipping SOLO la rama LLM');
    } else {
      const { browser: b2, page: p2 } = await newPage();
      try {
        await p2.goto(URL, { waitUntil: 'domcontentloaded' });
        await waitReady(p2, 'data-date-field-ready');
        await p2.waitForTimeout(400);
        const shot = await screenshot(p2, 'stagehand-date-field');
        const result = await sh.act(VISUAL_RUBRIC, { image: shot });
        const json = JSON.parse(result?.output ?? result?.text ?? '{}');
        const checks = ['sections_legible', 'label_visible', 'hint_visible', 'in_viewport', 'contraste'];
        const fails = checks.filter((c) => json?.[c] === 'FAIL');
        if (!fails || fails.length === 0) console.log('  ✓ date-field: visual rubric LLM PASS');
        else console.error(`  ✗ date-field: visual rubric LLM FAIL (${fails.join(', ')})`);
      } finally {
    ...
    console.error(`  ✗ date-field (LLM): ${String(err?.message ?? err)}`);
  }
}

const hardFailures = results.filter((r) => r.error).length;
report('date-field-stagehand', hardFailures === 0, { total: results.length, failures: hardFailures, results });