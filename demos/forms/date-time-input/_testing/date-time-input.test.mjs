// date-time-input.test.mjs — tests exhaustivos del demo date-time-input.html.
// Cobertura: smoke + funcional (open/close dialog, selección de fecha/hora,
...
tests.push({
  ...
  run: async (page) => {
    ...
    await screenshot(page, 'date-time-input-smoke');
  },
});
...
tests.push({
  ...
  run: async (page) => {
    ...
    assert.equal(data.prop.slice(0, 10), '2026-08-21', 'fecha debe reflejarse en la propiedad');
    assert.equal(data.fieldValue, '2026-08-21T15:45', 'el field interno debe tener el valor');
  },
});
...
tests.push({
  ...
  run: async (page) => {
    ...
    assert.equal(data.prop, '2027-02-09T03:00');
  },
});

tests.push({
  name: 'funcional: show() abre el dialog con paneles de fecha y hora',
  run: async (page) => {
    await page.goto(URL, { waitUntil: 'domcontentloaded' });
    await waitReady(page, 'data-date-time-input-ready');
    const result = await page.evaluate(async () => {
      const el = document.getElementById('basic');
      el.show();
      // Esperar a que el dialog abra (es un microtask).
...
tests.push({
  ...
  run: async (page) => {
    ...
    assert.ok(info.count >= 5, `esperaba >=5 secciones, hay ${info.count}`);
    assert.ok(info.labels.every(Boolean), 'todas las secciones deben tener aria-label');
    assert.ok(info.labels.some((l) => /año|year/i.test(l)), 'debe haber aria-label de año');
...
tests.push({
  name: 'edge: el panel móvil expone la barra de acciones (Limpiar/Ahora/Cancelar/Aceptar)',
  run: async (page) => {
    await page.goto(URL, { waitUntil: 'domcontentloaded' });
    await waitReady(page, 'data-date-time-input-ready');
    const info = await page.evaluate(() => {
...
tests.push({
  ...
  run: async (page) => {
    ...
    assert.equal(data.attr, '2026-11-11T11:11');
    assert.equal(data.prop, '2026-11-11T11:11');
  },
});
...
report('date-time-input', failures === 0, { total: tests.length, failures });
