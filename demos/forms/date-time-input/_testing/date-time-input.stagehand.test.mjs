// date-time-input.stagehand.test.mjs — rubric visual determinista para date-time-input.
//
...
const URL = `${BASE_URL}/demos/forms/date-time-input/date-time-input.html`;

const VISUAL_RUBRIC = `
Evalúa la calidad visual del campo de fecha y hora con panel que aparece en el screenshot.

Checklist (todas deben cumplirse; marca cada una PASS o FAIL):

1. CAMPO LEGIBLE: el campo de fecha y hora muestra sus secciones (día, mes, año, hora, minuto) claramente separadas. Cada sección es legible.

2. TRIGGER VISIBLE: hay un icono (reloj o calendario) al final del campo que invita a abrir el panel.

3. PANEL LEGIBLE: cuando se muestra el panel combinado, el calendario y la lista de horas son legibles y no se solapan.
...
async function checkDeterministic(page) {
  ...
  for (const f of data) {
    ...
    assert.ok(f.sectionsCount >= 5, `${tag}: debe haber >=5 secciones (y/m/d/h/M), hay ${f.sectionsCount}`);
...
try {
  ...
  console.log('  ✓ date-time-input: rubric determinista PASS');
  results.push({ name: 'date-time-input', skipped: false });
} catch (err) {
  console.error(`  ✗ date-time-input: ${String(err?.message ?? err)}`);
  try { await screenshot(page, 'fail-date-time-input'); } catch {}
  results.push({ name: 'date-time-input', error: String(err?.message ?? err) });
} finally {
...
if (process.env.STAGEHAND === '1') {
  ...
  try {
    ...
    if (!sh) {
      console.log('  ⊘ date-time-input: stagehand LLM sin credenciales, skipping SOLO la rama LLM');
    } else {
      const { browser: b2, page: p2 } = await newPage();
      try {
        await p2.goto(URL, { waitUntil: 'domcontentloaded' });
        await waitReady(p2, 'data-date-time-input-ready');
        await p2.evaluate(() => document.getElementById('basic').show());
        await p2.waitForTimeout(400);
        const shot = await screenshot(p2, 'stagehand-date-time-input');
        const result = await sh.act(VISUAL_RUBRIC, { image: shot });
        const json = JSON.parse(result?.output ?? result?.text ?? '{}');
        const checks = ['campo_legible', 'trigger_visible', 'panel_legible', 'in_viewport', 'contraste'];
        const fails = checks.filter((c) => json?.[c] === 'FAIL');
        if (!fails || fails.length === 0) console.log('  ✓ date-time-input: visual rubric LLM PASS');
        else console.error(`  ✗ date-time-input: visual rubric LLM FAIL (${fails.join(', ')})`);
      } finally {
    ...
    console.error(`  ✗ date-time-input (LLM): ${String(err?.message ?? err)}`);
  }
}

const hardFailures = results.filter((r) => r.error).length;
report('date-time-input-stagehand', hardFailures === 0, { total: results.length, failures: hardFailures, results });
