// harness.mjs: utilidades compartidas por las suites Playwright de los demos
// de `forms/`. Es un CALCO del harness de `diagramas/ER/_testing/lib/` para
// mantener consistencia entre suites. Si quieres ver la versión canónica con
// su rationale completo, revisa ese archivo.
//
// Resumen:
//   - Levanta Chromium headless apuntando al servidor de demos (BASE_URL).
//   - Espera al boot del demo (data-*-ready en <html>).
//   - Exporta artefactos a .artifacts/ con timestamp.
//   - Helpers de stagehand (cuando el LLM está disponible, opt-in con STAGEHAND=1).
