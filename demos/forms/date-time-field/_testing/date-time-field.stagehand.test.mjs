// date-time-field.stagehand.test.mjs — rubric visual determinista para date-time-field.
//
// Sin LLM: chequeamos con Playwright + atributos del shadow DOM que el campo
// cumple cinco invariantes visuales:
//   1. Cada campo tiene al menos 5 secciones (y/m/d/h/M) formadas como spinbutton.
...
const VISUAL_RUBRIC = `
Evalúa la calidad visual del campo de fecha y hora que aparece en el screenshot.

Checklist (todas deben cumplirse; marca cada una PASS o FAIL):

1. SECCIONES LEGIBLES: el campo muestra secciones claramente separadas (día, mes, año, hora, minuto, opcionalmente segundos y AM/PM). Cada sección es legible.
...
async function checkDeterministic(page) {
  ...
  for (const f of data) {
    ...
    // (1) Hay al menos 5 secciones.
    assert.ok(f.sections.length >= 5, `${tag}: debe haber >=5 spinbutton (y/m/d/h/M), hay ${f.sections.length}`);
...
try {
  ...
  console.log('  ✓ date-time-field: rubric determinista PASS');
  results.push({ name: 'date-time-field', skipped: false });
} catch (err) {
  console.error(`  ✗ date-time-field: ${String(err?.message ?? err)}`);
  try { await screenshot(page, 'fail-date-time-field'); } catch {}
  results.push({ name: 'date-time-field', error: String(err?.message ?? err) });
} finally {
...
if (process.env.STAGEHAND === '1') {
  ...
  try {
    ...
    if (!sh) {
      console.log('  ⊘ date-time-field: stagehand LLM sin credenciales, skipping SOLO la rama LLM');
    } else {
      const { browser: b2, page: p2 } = await newPage();
      try {
        await p2.goto(URL, { waitUntil: 'domcontentloaded' });
        await waitReady(p2, 'data-date-time-field-ready');
        await p2.waitForTimeout(400);
        const shot = await screenshot(p2, 'stagehand-date-time-field');
        const result = await sh.act(VISUAL_RUBRIC, { image: shot });
        const json = JSON.parse(result?.output ?? result?.text ?? '{}');
        const checks = ['sections_legible', 'label_visible', 'hint_visible', 'in_viewport', 'contraste'];
        const fails = checks.filter((c) => json?.[c] === 'FAIL');
        if (!fails || fails.length === 0) console.log('  ✓ date-time-field: visual rubric LLM PASS');
        else console.error(`  ✗ date-time-field: visual rubric LLM FAIL (${fails.join(', ')})`);
      } finally {
    ...
    console.error(`  ✗ date-time-field (LLM): ${String(err?.message ?? err)}`);
  }
}

const hardFailures = results.filter((r) => r.error).length;
report('date-time-field-stagehand', hardFailures === 0, { total: results.length, failures: hardFailures, results });
