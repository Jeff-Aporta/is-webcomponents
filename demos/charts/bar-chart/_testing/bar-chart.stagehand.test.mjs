// bar-chart.stagehand.test.mjs — visual rubric determinista (Playwright puro).
...
const DEMO = {
  url: `${BASE_URL}/demos/charts/bar-chart/bar-chart.html`,
  readyAttr: 'data-bar-chart-ready',
  name: 'bar-chart',
};
...
async function checkDeterministic(page) {
  ...
  // (4) GEOMETRÍA DE BARRAS — para bar-chart, los marks .mark-bar deben
  //     tener altura > 0 y estar todos dentro del área del chart.
  const bars = data.marks.filter((m) => m.cls.includes('mark-bar'));
  assert.ok(bars.length > 0, `esperaba al menos 1 barra, hay ${bars.length}`);
  for (const b of bars) {
