...
// here = demos/charts/_global/lib/
// ../.. = demos/charts/_global/
// El REPO_ROOT se mantiene coherente con la convención del runner:
export const REPO_ROOT = join(here, '..', '..', '..');
// El ARTIFACTS_DIR se calcula desde el directorio _testing del consumer
// (que es quien llama a screenshot/report). Lo guardamos como global para
// que cualquier sub-test del mismo demo escriba en su propia carpeta.
...
/** Captura screenshot con timestamp en la carpeta .artifacts del consumer. */
export async function screenshot(page, name) {
  const ts = new Date().toISOString().replace(/[:.]/g, '-');
  // Apuntamos al .artifacts del subdirectorio _testing/ del consumer:
  const dir = join(here, '..', '..', '..', '.artifacts');
  await mkdir(dir, { recursive: true });
  const path = join(dir, `${name}-${ts}.png`);
