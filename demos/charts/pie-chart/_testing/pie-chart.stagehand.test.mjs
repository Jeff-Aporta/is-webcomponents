// pie-chart.stagehand.test.mjs — visual rubric determinista (Playwright puro).
...
const DEMO = {
  url: `${BASE_URL}/demos/charts/pie-chart/pie-chart.html`,
  readyAttr: 'data-pie-chart-ready',
  name: 'pie-chart',
};
...
async function checkDeterministic(page) {
  ...
  // (4) GEOMETRÍA DE REBANADAS — para pie-chart, los paths .mark-slice
  //     deben contener comandos de arc (A) y todos juntos deben cubrir
  //     el círculo completo (la suma de sweeps ≈ 2π). Las slices no deben
  //     solaparse más allá del sliceGap visual.
  const slices = data.marks.filter((m) => m.cls.includes('mark-slice'));
  assert.ok(slices.length > 0, `esperaba al menos 1 slice, hay ${slices.length}`);
  for (const s of slices) {
    assert.ok(/[Aa]/.test(s.d), `slice debe contener arc A: "${s.d.slice(0, 60)}"`);
    assert.ok(s.w > 0 && s.h > 0, `slice debe tener tamaño > 0: ${s.w}×${s.h}`);
  }
}
