// component-diagram.stagehand.test.mjs — verificaciones de calidad visual
// (Nivel 1) para el demo component-diagram.html.
import assert from 'node:assert/strict';
import { BASE_URL, newPage, close, waitReady, screenshot, report, maybeStagehand } from './lib/harness.mjs';

const DEMO = {
  url: `${BASE_URL}/demos/diagramas/component-diagram/component-diagram.html`,
  readyAttr: 'data-component-diagram-ready',
...
async function checkDeterministic(page) {
  ...
  const data = await page.evaluate(() => {
    ...
    const svg = shadow.querySelector('svg.cd-svg');
    const svgRect = svg.getBoundingClientRect();
    const nodes = [...shadow.querySelectorAll('[data-cmp-id]')].map((g) => {
      const r = g.getBoundingClientRect();
      return { id: g.dataset.cmpId, x: r.x, y: r.y, w: r.width, h: r.height };
    });
    const edges = [...shadow.querySelectorAll('.cd-edge path')].map((p) => ({
      d: p.getAttribute('d') ?? '',
  ...
  assert.equal(overlaps.length, 0, `componentes solapados: ${JSON.stringify(overlaps)}`);
  ...
  for (const n of data.nodes) {
    ...
    assert.ok(inside, `componente ${n.id} se sale del SVG`);
  }
