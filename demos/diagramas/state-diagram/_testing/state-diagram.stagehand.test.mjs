// state-diagram.stagehand.test.mjs — verificaciones de calidad visual
// (Nivel 1) para el demo state-diagram.html.
import assert from 'node:assert/strict';
import { BASE_URL, newPage, close, waitReady, screenshot, report, maybeStagehand } from './lib/harness.mjs';

const DEMO = {
  url: `${BASE_URL}/demos/diagramas/state-diagram/state-diagram.html`,
  readyAttr: 'data-state-ready',
  name: 'state-diagram',
};
...
async function checkDeterministic(page) {
  ...
  const data = await page.evaluate(() => {
    ...
    const svg = shadow.querySelector('svg.st-svg');
    const svgRect = svg.getBoundingClientRect();
    const nodes = [...shadow.querySelectorAll('.st-node')].map((g) => {
      const r = g.getBoundingClientRect();
      return { id: g.dataset.nodeId, x: r.x, y: r.y, w: r.width, h: r.height };
    });
    const edges = [...shadow.querySelectorAll('.st-trans path')].map((p) => ({
      d: p.getAttribute('d') ?? '',
      stroke: p.getAttribute('stroke') ?? '',
      computedStroke: getComputedStyle(p).stroke ?? '',
    }));
    const texts = [...shadow.querySelectorAll('.st-node text')].map((t) => ({
  ...
  assert.equal(overlaps.length, 0, `estados solapados: ${JSON.stringify(overlaps)}`);
  ...
  for (const n of data.nodes) {
    ...
    assert.ok(inside, `estado ${n.id} se sale del SVG`);
  }
