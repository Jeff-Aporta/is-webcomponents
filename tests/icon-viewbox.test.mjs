...
//
// Reparar / regenerar snapshot:  node scripts/fix-icon-viewbox.mjs
//
// Uso:  node tests/icon-viewbox.test.mjs
...
try { snap = await readJson('viewbox.snapshot.json'); }
  ...
  console.log('SKIP icon-viewbox — falta viewbox.snapshot.json (corre `node scripts/fix-icon-viewbox.mjs`)');
  process.exit(0);
}
...
assert.equal(
  ...
  'SVG con viewBox distinto al del snapshot — alguien reescribio los headers.\n' +
    'Si el cambio es legitimo, regenera con `node scripts/fix-icon-viewbox.mjs`.\n  ' +
...
assert.equal(
  sinMeta.length,
  0,
  `collections.json no cubre estas familias (corre \`node scripts/sync-icon-collections.mjs\`): ${sinMeta.join(', ')}`,
);
