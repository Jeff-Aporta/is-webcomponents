...
import { placeEdgeActors, pointAtFraction, parsePathPoints } from '../src/components/_shared/diagram-edge-actors.ts';
import { spreadOrthogonalPaths } from '../src/components/_shared/diagram-edge-spread.ts';
...
if (failures.length) {
  console.error('diagram-edge-actors.test.ts: FAIL');
  for (const f of failures) console.error('  -', f);
  process.exit(1);
}
console.log('diagram-edge-actors.test.ts: PASS');
