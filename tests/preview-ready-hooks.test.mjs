// tests/preview-ready-hooks.test.mjs
//
...
//
// Uso: node tests/preview-ready-hooks.test.mjs
...
const chrome = leer('src/components/layout/preview-component.js');
if (!chrome.includes(`'${EVENTO}'`)) {
  failures.push(`preview-component.js ya no emite ${EVENTO}`);
}
if (!/detail:\s*\{\s*tag:/.test(chrome)) {
  failures.push(`preview-component.js: el detail de ${EVENTO} debe traer el tag (cdn-panel.js lo necesita)`);
}
...
if (failures.length) {
  console.error(`preview-ready-hooks.test.mjs: FAIL — ${failures.length}\n`);
  for (const f of failures) console.error(`  - ${f}`);
  process.exit(1);
}

console.log(`preview-ready-hooks.test.mjs: PASS — ${Object.keys(oyentes).length} módulos reenganchados a ${EVENTO}`);
