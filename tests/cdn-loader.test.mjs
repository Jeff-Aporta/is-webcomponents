/**
 * tests/cdn-loader.test.mjs — contrato del ISWebComponentsLoader.
 */
...
const src = join(root, 'src', 'cdn', 'loader.js');
const dist = join(root, 'dist', 'cdn', 'loader.min.js');
const indexHtml = readFileSync(join(root, 'index.html'), 'utf8');

test('src/cdn/loader.js expone API pública + mirrors/pin + has/getLoaded', () => {
...
test('dist/cdn/loader.min.js y loader.md existen; banner con docs', () => {
  assert.ok(existsSync(dist), 'falta dist/cdn/loader.min.js — corre npm run build');
  assert.ok(existsSync(join(root, 'dist', 'cdn', 'loader.md')), 'falta dist/cdn/loader.md');
...
test('README CDN documenta pin y mirrors', () => {
  ...
  assert.match(readme, /loadCSSBase/);
  assert.match(readme, /pin\(/);
  assert.match(readme, /mirrors|jsDelivr|Pages/i);
});
