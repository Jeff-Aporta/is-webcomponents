...
test('ISComponentPreview.on no crashea con target null', () => {
  const src = read('src/previews/_kit/ISComponentPreview.ts');
...
test('url-nav escribe solo dentro de ?s= (STATE_PARAM s)', () => {
  const src = read('src/components/_shared/url-nav.ts');
...
test('format-bytes autofit', () => {
  const fb = read('src/components/helpers/format-bytes.ts');
  assert.match(fb, /autofit/);
  assert.match(fb, /n\s*\/\s*1024\s*>=\s*1|scaleAutofit/);

  const snip = read('src/components/feedback/cdn-snippet.ts');
  assert.doesNotMatch(snip, /sizes\.json|totalCdnSize|cdn-sizes/);
});
...
test('is-btn-ref / is-catalogo-gen behaviors asignan controller', () => {
  for (const tag of ['is-btn-ref', 'is-catalogo-gen']) {
    ...
    assert.ok(
      ...
      `${tag}: el controller debe traer Lista (inline o vía controller-from-config.ts)`,
    );
  }
