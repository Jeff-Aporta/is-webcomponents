...
async function loadIndex(prefix) {
  try {
    const txt = await readFile(join(root, 'dist', 'assets', 'icons', `${prefix}.json`), 'utf8');
...
for (const { prefix, name } of refs) {
  const url = `${BASE}/dist/assets/icons/${prefix}/${name}.svg`;
  try {
...
console.log(`cdn-icons.test.ts: PASS — ${okCount}/${results.length} iconos servidos como SVG local`);
process.exit(0);