// tests/component-audit.test.mjs
//
// Higiene de componentes: corre scripts/audit-components.mjs y falla si
...
//
// Uso:  node tests/component-audit.test.mjs
...
let out = '';
try {
  out = execFileSync(process.execPath, [join(root, 'scripts', 'audit-components.mjs')], {
    cwd: root,
    encoding: 'utf8',
  });
} catch (e) {
  console.error('audit-components.mjs no pudo ejecutarse:', e.message);
  process.exit(1);
}
...
console.log(`component-audit.test.mjs: PASS — ${total} componentes sin fugas de listeners/observers ni registros rotos`);
process.exit(0);
