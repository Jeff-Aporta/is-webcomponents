...
//
// Uso:  node tests/attr-enums.test.ts
...
const componentFiles = await walk(join(root, 'src', 'components'), /\.ts$/);

/** Vocabulario compartido, leído de su fuente real para que no se duplique aquí. */
const SHARED = {
  INTENT: [...(await readFile(join(root, 'src/components/_shared/intent.ts'), 'utf8'))
    .match(/export const INTENT = Object\.freeze\(\[([\s\S]*?)\]\)/)[1]
    .matchAll(/'([^']+)'/g)].map((m) => m[1]),
  TONE: [...(await readFile(join(root, 'src/components/_shared/tone.ts'), 'utf8'))
...
for (const file of componentFiles) {
  ...
  // compartido de `_shared/intent.ts` / `_shared/tone.ts`:
  //
...
const buttonJs = await readFile(join(root, 'src/components/actions/button.ts'), 'utf8');
if (/variant\s+[^\n]*\bghost\b/.test(buttonJs)) {
  assert.match(
    buttonCss,
    /\[variant="ghost"\]/,
    'button.ts documenta la variante `ghost` pero button.css no la implementa',
  );
  assert.match(
    buttonCss,
    /--_text-hover/,
