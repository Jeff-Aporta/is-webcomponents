...
import { computeComponentLayout, resolveComponentSpec, LOLLI_R, LOLLI_GAP, parseHttpEndpoint } from '../src/components/diagrams/component-spec.ts';
import { parsePathPoints } from '../src/components/_shared/diagram-edge-actors.ts';
import { COL_GUTTER, EDGE_CLEARANCE, orthoPolysOverlap, pathHasDiagonal, pathIllegal, pathShareLen, pointInOrtho, segsFromPath, segmentoCortaCaja } from '../src/components/diagrams/component-pack.ts';
...
if (failures.length) {
  console.error('component-diagram-ifaces.test.ts: FAIL');
  for (const f of failures) console.error('  -', f);
  process.exit(1);
}

console.log(`component-diagram-ifaces.test.ts: PASS — ${layout.interfaces.length}+${synLayout.interfaces.length} interfaces, ${layout.edges.length}+${synLayout.edges.length} aristas`);
process.exit(0);
