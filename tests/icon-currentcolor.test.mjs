// tests/icon-currentcolor.test.mjs
//
...
//
// Reglas:
//   1. components/media/icon.js importa `resolveIconRaw` del iconify-loader.
...
//
// Uso:  PORT=8391 node tests/icon-currentcolor.test.mjs
...
assert.ok(
  ...
  'icon.js debe importar `resolveIconRaw` (no `resolveIconSvg`)'
);
assert.ok(
  !/resolveIconSvg/.test(iconJs),
  'icon.js ya no debe usar resolveIconSvg: el SVG debe inyectarse inline'
);
assert.ok(
  /class="inline"/.test(iconJs),
  'icon.js debe declarar `<span class="inline">` en el template'
);
assert.ok(
  !/<img class="img"/.test(iconJs),
  'icon.js no debe usar <img class="img"> para cargar SVGs (rompe currentColor)'
);
assert.ok(
  /#normalizeInlineSvg|#normalizeSvg/.test(iconJs),
  'icon.js debe tener una funcion que fuerce fill/stroke a currentColor'
);
assert.ok(
  /\.inline/.test(iconCss),
