...
// como `\uXXXX` literal.
//
// Uso: node tests/preview-blocks.test.ts
...
/** Lenguajes que entiende `resolveMode` en highlight-code.ts. */
...
if (failures.length) {
  console.error(`preview-blocks.test.ts: FAIL — ${failures.length}\n`);
  for (const f of failures.slice(0, 40)) console.error(`  - ${f}`);
  if (failures.length > 40) console.error(`  … y ${failures.length - 40} más`);
  process.exit(1);
}

console.log(`preview-blocks.test.ts: PASS — ${codigo} bloques de código con lang coherente y ${tablas} tablas con encabezados de texto`);
