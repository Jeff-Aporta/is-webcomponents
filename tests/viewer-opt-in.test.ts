...
//
// Uso:  node tests/viewer-opt-in.test.ts
...
const conVisor = scanned.filter((f) => f.endsWith('.ts') && readFileSync(f, 'utf8').includes('#openOwnViewer'));
...
console.log(
  `viewer-opt-in.test.ts: PASS — ${conVisor.length} componentes con visor, todos opt-in; `
