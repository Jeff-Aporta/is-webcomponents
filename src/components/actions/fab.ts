...
(() => {
  ...
  class IsFab extends ElementBase {
    ...
    constructor() {
      ...
      this.#root = shadow.querySelector<HTMLElement>('is-button')!;
    }
    ...
    get position(): Position {
      ...
      return v != null && (VALID_POSITION as readonly string[]).includes(v)
        ? (v as Position) : 'bottom-end';
    }
    set position(v: Position | null | undefined) {
      if (v == null || String(v) === '') this.removeAttribute('position');
      else if ((VALID_POSITION as readonly string[]).includes(v)) this.setAttribute('position', v);
    }

    get color(): string {
      const v = this.getAttribute('color');
      return v != null && VALID_COLOR.includes(v) ? v : 'brand';
    }
    set color(v) {
    ...
    #sync() {
      ...
      if (icon) {
        ...
        if (slot && slot.assignedNodes().length === 0) {
          const ic = this.shadowRoot!.querySelector<HTMLElement>('is-icon')!;
      ...
      const slot = this.#root.querySelector<HTMLSlotElement>(':scope > slot:not([name])');
      if (slot) {
