...
(() => {
  ...
  class IsDropdownItem extends withStyleAttrs(HTMLElement) {
    ...
    constructor() {
      ...
      this.#row = shadow.querySelector<HTMLElement>('.item')!;
      this.#check = shadow.querySelector<HTMLElement>('.checkmark')!;
      this.#submenuIcon = shadow.querySelector<HTMLElement>('.submenu-icon')!;
      this.#submenu = shadow.querySelector<HTMLElement>('.submenu')!;
      if (SUPPORTS_POPOVER) this.#submenu.popover = 'manual';
      this.#submenuSlot = shadow.querySelector<HTMLSlotElement>('slot[name="submenu"]')!;
    ...
    closeSubmenu() {
      ...
      this.#submenuSlot.assignedElements({ flatten: true }).forEach((el) => {
        const hijo = el as HTMLElement & { closeSubmenu?: () => void };
        if (hijo.closeSubmenu) hijo.closeSubmenu();
      });
    }
