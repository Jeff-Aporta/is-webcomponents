  ...
  class IsCopyButton extends withStyleAttrs(HTMLElement) {
    ...
    constructor() {
      ...
      this.#icons = {
        ...
        success: shadow.querySelector<HTMLElement>('[data-state="success"]')!,
        error: shadow.querySelector<HTMLElement>('[data-state="error"]')!
      };
      if ('attachInternals' in this) {
        try { this.#internals = this.attachInternals(); } catch { /* noop */ }
      }
      shadow.querySelector<HTMLSlotElement>('slot:not([name])')!.addEventListener('slotchange', () => this.#onSlotChange());
      this.#btn.addEventListener('click', (e: Event) => { e.stopPropagation(); this.#handleCopy(); });
      shadow.querySelector<HTMLElement>('.trigger')!.addEventListener('click', (e: Event) => {
        if (this.#hasCustom && !this.#btn.contains(e.target as Node | null)) this.#handleCopy();
      });
    }
    ...
    get tooltipPlacement(): string {
      ...
      return v != null && PLACEMENTS.includes(v) ? v : 'top';
    }
    set tooltipPlacement(v: string) { this.setAttribute('tooltip-placement', v); }
    ...
    #setState(success: boolean, error: boolean): void {
    ...
    #wireHover() {
      ...
      const anchor = this.shadowRoot!.querySelector<HTMLElement>('.trigger')!;
    ...
    async #handleCopy() {
      ...
      if (this.from) {
        ...
        // `getRootNode()` puede devolver Document o ShadowRoot; solo el primero
        // y el segundo tienen `getElementById`, y el tipo comun no lo declara.
        ...
        // `from` puede nombrar una propiedad arbitraria del destino: el acceso
        // por indice es el punto del contrato, no un descuido de tipos.
        else if (isProperty) valueToCopy = String((target as unknown as Record<string, unknown>)[field] ?? '');
    ...
    async #showStatus(status: 'success' | 'error'): Promise<void> {
      this.#status = status;
