...
class IsDiagramLightbox extends IsLightbox {
  ...
  #installDiagramToolbar() {
    const bar = this.shadowRoot.querySelector('.lb-bar');
    if (!bar) return;

    const lead = bar.querySelector('.lb-bar__lead');
    ...
    const trail = bar.querySelector('.lb-bar__trail');
    ...
    trail.insertBefore(codeBtn, trail.querySelector('[data-act="zoom-reset"]'));

    this.#btnPlay = nav.querySelector('[data-act="play"]');
    this.#btnStop = nav.querySelector('[data-act="stop"]');
    this.#ring = nav.querySelector('.lb-ring__fill');
    this.#ringLabel = nav.querySelector('.lb-step');

    // Mostrar los botones por defecto que sí tienen sentido en diagramas.
    const share = trail.querySelector('[data-act="share"]');
    if (share) share.hidden = false;

    const codePanel = this.shadowRoot.querySelector('.lb-code');
    codePanel.innerHTML = `
  ...
  #onClick = (e) => {
    ...
    switch (btn.dataset.act) {
      ...
      case 'code-cancel':
        this.shadowRoot.querySelector('.lb-code').hidden = true;
        e.stopImmediatePropagation();
        break;
  ...
  #mountDiagram() {
    ...
    const host = this.shadowRoot.querySelector('.lb-host');
    if (!host) return;
  ...
  #openCode() {
    const area = this.shadowRoot.querySelector('.lb-code__area');
    const err = this.shadowRoot.querySelector('.lb-code__err');
    area.value = diagramCodeJson(this.kind, this.#basePayload);
    err.hidden = true;
    this.shadowRoot.querySelector('.lb-code').hidden = false;
  }
