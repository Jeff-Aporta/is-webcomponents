...
function readTask(raw, i: number) {
 ...
 */
export function computeGanttLayout(spec: object, opts = {}) {
  ...
  const longestLabel = tasks.reduce((acc: number, t) => Math.max(acc, richTextPlain(t.label).length), 4);
