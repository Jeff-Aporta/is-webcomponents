...
function packPackage(pkg, kids, gut = COL_GUTTER, rowGap = ROW_GAP) {
...
export function pointInOrtho(pts, x, y) {
...
export function segmentoEsDiagonal(x1, y1, x2, y2) {
...
export function segmentoCortaCaja(x1, y1, x2, y2, c) {
...
function laneInGap(gap, rank, total) {
...
function gridRoute(from, to, boxes, clearance) {
  ...
  const near = (x, y, tx, ty) => Math.abs(x - tx) + Math.abs(y - ty) <= step * 2;
...
function overshootsTip(pts) {
  ...
  const between = (u, v, m) => m > Math.min(u, v) + 0.5 && m < Math.max(u, v) - 0.5;
...
function wrapCandidates(from, to, a0, b0, aJog, bJog, boxes, pad, lane = 0, clamp) {
 ...
 */
export function routeAvoidingBoxes(from, to, obstaculos, rank = 0, total = 1, opts = {}) {
