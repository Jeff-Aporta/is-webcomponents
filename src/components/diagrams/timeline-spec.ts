 ...
 */
function compressedEventScale(sortedMs: number[], r0: number, maxGapPx: number) {
  ...
  const cappedMs = rawGaps.map((g: number) => Math.min(g, median * 3));
...
function readEvent(raw, i: number) {
 ...
 */
export function computeTimelineLayout(spec: object, opts = {}) {
