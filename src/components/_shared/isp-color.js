 ...
 * @returns {string | null}
 */
export function resolveMixWith(raw) {
  if (raw == null || raw === '') return null;
  const s = String(raw).trim();
  if (!s) return null;
 ...
 */
export function classifyColor(raw) {
 ...
 * }} opts
 */
export function syncIspColor(el, opts) {
  ...
  if (
    ...
    el.setAttribute('color', /** @type {string} */ (classified.value));
  }
  ...
  if (classified.kind === 'none') {
    ...
    el.style.setProperty(colorVar, /** @type {string} */ (classified.value));
...
export function isSemanticColor(raw) {
