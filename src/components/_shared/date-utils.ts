...
export function isoOf(year: string, month: number, day) {
...
export function to12Hour(h: number) {
  const meridiem = h < 12 ? 'AM' : 'PM';
  const hour = h % 12 === 0 ? 12 : h % 12;
  return { hour, meridiem };
}

export function from12Hour(hour: number, meridiem) {
  const h = hour % 12;
  return meridiem === 'PM' ? h + 12 : h;
}

/** Une fecha y hora en el valor compuesto que usan los campos date-time. */
export function joinDateTime(iso: string, time: string) {
  if (!iso) return '';
