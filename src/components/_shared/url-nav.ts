 ...
 */
export function patchUrlState(patch: Record<string, unknown>) {
 ...
 * @returns {string | null}
 */
export function readUrlNav(key: string) {
 ...
 */
export function writeUrlNav(key: string, value: string | null | undefined) {
  const k = String(key || '').trim();
  if (!k) return;
