...
(() => {
  ...
  class IsOffscreenCanvas extends HTMLElement {
    static get observedAttributes() { return ['width', 'height', 'worker-src']; }
    ...
    constructor() {
      ...
      this.#cv = shadow.querySelector('.cv');
    }

    connectedCallback() { this.#boot(); }
    disconnectedCallback() { this.#worker?.terminate(); this.#worker = null; }
