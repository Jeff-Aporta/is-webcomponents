...
export class IsAgGrid extends ElementBase {
  ...
  #cacheRefs() {
    ...
    this.#viewport = root.querySelector('.mim-dg__viewport');
    this.#headerRow = root.querySelector('.mim-dg__header-row');
    this.#body = root.querySelector('.mim-dg__body');
    this.#countEl = root.querySelector('.mim-dg__count');
    this.#pagerInfo = root.querySelector('.mim-dg__pager-info');
    this.#pageSizeSelect = root.querySelector('.mim-dg__page-size-select');
    this.#densityBtns = root.querySelectorAll('.mim-dg__density-btn');
    this.#toolbar = root.querySelector('.mim-dg__toolbar');
    this.#groupPanel = root.querySelector('.mim-dg__group-panel');
    this.#groupChips = root.querySelector('.mim-dg__group-chips');
  }
  ...
  #renderDensity() {
    ...
    this.shadowRoot.querySelector('.mim-dg').dataset.density = this.#density;
  }
  ...
  #renderGroupPanel() {
    ...
    this.#groupPanel.querySelector('.mim-dg__group-hint').style.display = state.rowGroupCols.length ? 'none' : '';
  }
  ...
  #buildFilterFromPopover(col, pop) {
    ...
    if (ft === 'set') {
      const allValues = uniqueValuesSafe(this.#api.getAllRows(), col);
      const selected = [...pop.querySelectorAll('[data-set-checkbox]')]
        .filter((cb) => cb.dataset.checked === 'true')
        .map((cb) => cb.closest('[data-set-val]').dataset.setVal);
