...
(() => {
  ...
  class IsDigitalClock extends ElementBase {
    ...
    #renderList() {
      ...
      if (!anyTabbable) {
        const first = list.querySelector<HTMLElement>('.opt:not([disabled])');
    ...
    #column(section, label, items) {
      ...
      if (!col.querySelector<HTMLElement>('[tabindex="0"]')) {
        const first = col.querySelector<HTMLElement>('.opt:not([disabled])');
        if (first) first.tabIndex = 0;
      }
      return col;
    }
    ...
    #onKey = (e: KeyboardEvent) => {
      ...
      if (!step) return;
      e.preventDefault();
      const siblings = [...btn.parentElement.querySelectorAll<HTMLElement>('.opt:not([disabled])')];
