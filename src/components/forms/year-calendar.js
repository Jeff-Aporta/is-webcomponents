...
(() => {
  ...
  class IsYearCalendar extends ElementBase {
    ...
    focus(opts) {
      (this.#base.querySelector('[tabindex="0"]') || this.#base.firstElementChild)?.focus(opts);
    }

    /** Deja el año activo centrado: la lista puede abarcar dos siglos. */
    scrollToSelection() {
      const el = this.#base.querySelector('[data-selected], [data-current]');
    ...
    #moveFocus(year) {
      const el = this.#base.querySelector(`[data-year="${year}"]`);
