...
(() => {
  ...
  class IsPinInput extends ElementBase {
    ...
    constructor() {
      ...
      this.#hiddenInput = shadow.querySelector<HTMLInputElement>('.hidden-input')!;
    }
    ...
    onConnected(): void {
      ...
      this.#render();
      this.#hiddenInput.addEventListener('input', this.#onHiddenInput = (): void => {
        // write-only sync
        this.#hiddenInput.value = '';
      });
      this.#cells.addEventListener('click', this.#onSlotClick = (e: PointerEvent): void => {
        const idx = this.#indexFromEvent(e);
        if (idx >= 0) this.#focusIndex(idx);
      });
      this.#cells.addEventListener('keydown', (e: KeyboardEvent) => this.#onKeyDown(e));
      this.#cells.addEventListener('input', (e: Event) => this.#onCellInput(e));
      this.#onPaste = (e: ClipboardEvent) => this.#handlePaste(e);
    ...
    #lengthArray(): unknown[] { return Array.from({ length: this.#length() }); }
    ...
    #render(): void {
      ...
      // Restaurar valores
      this.#values.forEach((v: string, i: number) => {
        const inp = this.#cells.querySelector<HTMLInputElement>(`input[data-index="${i}"]`);
        if (inp) inp.value = v;
      });
    }
    ...
    #syncState(): void {
      ...
      this.#cells.querySelectorAll<HTMLInputElement>('input').forEach((inp: HTMLInputElement) => {
    ...
    #indexFromEvent(e: Event): number {
      const target = (e.target as Element | null)?.closest('input.cell') as HTMLElement | null;
      if (!target) return -1;
      return parseInt(target.dataset.index ?? '-1', 10);
    }
    ...
    #setValue(text: string): void {
      ...
      this.#values = chars.concat(this.#lengthArray().map(() => '' as string)).slice(0, this.#length());
      this.#render();
    }

    #focusIndex(idx: number): void {
      const inp = this.#cells.querySelector<HTMLInputElement>(`input[data-index="${idx}"]`);
      if (inp) inp.focus();
    }
