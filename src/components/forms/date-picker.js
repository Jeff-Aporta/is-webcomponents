...
(() => {
  ...
  class IsDatePicker extends ElementBase {
    ...
    constructor() {
      ...
      this.#monthDd = shadow.querySelector('[data-jump="month"]');
      this.#yearDd = shadow.querySelector('[data-jump="year"]');
      this.#monthText = this.#monthDd.querySelector('.nav-select-text');
      this.#yearText = this.#yearDd.querySelector('.nav-select-text');
      this.#weekdays = shadow.querySelector('.weekdays');
      this.#grid = shadow.querySelector('.grid');
      this.#dayView = shadow.querySelector('.day-view');
      this.#monthView = shadow.querySelector('.month-view');
      this.#yearView = shadow.querySelector('.year-view');
    ...
    get disabledDays() {
      return new Set(
        String(this.getAttribute('disabled-days') || '')
          .split(/[\s,]+/).filter(Boolean).map(Number).filter((n) => n >= 0 && n <= 6),
      );
    }
    set disabledDays(v) {
    ...
    focusDate(iso) {
      ...
      this.#grid.querySelector(`[data-iso="${iso}"]`)?.focus();
    }
    ...
    #wireJump(dd, kind) {
      ...
      dd.addEventListener('is-after-show', () => {
        ...
        requestAnimationFrame(() => {
          const active = dd.querySelector('is-dropdown-item[checked]');
          if (!active) return;
          active.focus();
    ...
    #setJumpItems(dd, items) {
      for (const old of dd.querySelectorAll(':scope > is-dropdown-item')) old.remove();
      dd.append(...items);
    }

    #jumpItem(value, label, active, disabled) {
    ...
    #paintPreview() {
      ...
      for (const cell of this.#grid.querySelectorAll('.day')) {
