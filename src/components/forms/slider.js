...
(() => {
  ...
  class IsSlider extends ElementBase {
    ...
    #readAttrValues() {
      ...
      const parts = String(raw).split(',').map((s) => Number(s.trim())).filter(Number.isFinite);
    ...
    #render() {
      ...
      for (const dot of this.#marksEl.querySelectorAll('.mark')) {
      ...
      this.#thumbs.forEach((thumb, i) => {
        ...
        const bubble = thumb.querySelector('.value-label');
    ...
    #nudge(index, delta, commitChange) {
