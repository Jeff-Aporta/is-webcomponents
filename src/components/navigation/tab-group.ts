...
(() => {
  ...
  class IsTabGroup extends withStyleAttrs(HTMLElement) {
    ...
    constructor() {
      ...
      this.#navSlot = shadow.querySelector<HTMLSlotElement>('slot[name="nav"]')!;
      this.#tabsWrap = shadow.querySelector<HTMLElement>('.tabs')!;
      this.#scrollStart = shadow.querySelector<HTMLElement>('.scroll-start')!;
      this.#scrollEnd = shadow.querySelector<HTMLElement>('.scroll-end')!;
    ...
    #panelNames() {
      return new Set(this.#allPanels().map((p: HTMLElement) => p.getAttribute('name')).filter(Boolean));
    }
  ...
  class IsTab extends HTMLElement {
    ...
    #syncState() {
      const btn = this.shadowRoot!.querySelector<HTMLButtonElement>('button.tab');
      const close = this.shadowRoot!.querySelector<HTMLElement>('[data-tab-close]');
