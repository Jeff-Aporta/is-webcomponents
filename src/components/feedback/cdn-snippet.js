...
/**
 * <is-cdn-snippet> — panel CDN + docs para agentes (sin npm/npx).
 *
 * Atributos
 ...
 *
 * Filas CDN: CSS común → JS del tag → alternativa categoría → deps → all.
 * Bloque agentes: prompt CDN-first + enlaces raw a MD (módulo / categoría / índice).
 */
(() => {
  ...
  TEMPLATE.innerHTML = /* html */ `
    <section class="cdn" aria-label="Enlaces CDN">
      <header class="cdn__head">
        ...
        <p class="cdn__hint">
          Carga el kit desde jsDelivr. Primero el CSS común; luego el JS del
          componente (o el bundle de categoría / <code>all.min.js</code>).
          Cada tag se registra en <code>customElements</code>.
        </p>
      </header>
      <ol class="cdn__list">
  ...
  class IsCdnSnippet extends HTMLElement {
    ...
    #mounted = false;
    #urls = { single: '', category: '', all: '', llmPrompt: LLM_PROMPT };
