...
test('4. OBSERVED incluye level, color, mix, mix-with, size', () => {
...
test('6. color cubre brand | neutral | info | success | warning | danger | current', () => {
  const src = readFileSync(TS, 'utf8');
...
test('7. usa isp-color helpers (classifyColor, normalizeMix, syncIspColor)', () => {
  const src = readFileSync(TS, 'utf8');
...
test('8. extiende ElementBase (cambios de atributo se procesan)', () => {
  const src = readFileSync(TS, 'utf8');
