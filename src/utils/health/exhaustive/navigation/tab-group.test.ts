...
test('4. OBSERVED incluye active, placement, activation', () => {
...
test('5. placement acepta top | bottom | start | end', () => {
  const src = readFileSync(TS, 'utf8');
...
test('6. activation acepta auto | manual', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/['"]auto['"]/.test(src) && /['"]manual['"]/.test(src));
});

test('7. persiste active en URL via url-key (readUrlNav/writeUrlNav)', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/readUrlNav/.test(src) && /writeUrlNav/.test(src));
});

test('8. emite is-change cuando cambia el active', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/['"]is-change['"]/.test(src));
});

test('9. integra con is-tab e is-tab-panel (slots)', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/<slot[^>]*name\s*=\s*['"]nav['"]/.test(src), 'slot nav para is-tab');
  assert.ok(/<slot[^>]*name\s*=\s*['"]panel['"]/.test(src) || /<slot>/.test(src));
});

test('10. registra <is-tab> y <is-tab-panel>', () => {
  const src = readFileSync(TS, 'utf8');
...
test('11. custom element is-tab-group registrado', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/defineElement\s*\(\s*['"]is-tab-group['"]/.test(src));
});

test('12. JSON tiene demo(s) y reference', () => {
  const json = JSON.parse(readFileSync(JSON_PATH, 'utf8'));
