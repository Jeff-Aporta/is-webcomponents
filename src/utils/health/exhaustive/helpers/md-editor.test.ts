...
test('4. OBSERVED incluye label, placeholder, filename, value', () => {
...
test('5. usa textarea (no contenteditable para markdown)', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/<textarea[\s\S]*?>/.test(src), 'shadow debe tener <textarea>');
});

test('6. expone getter/setter para value', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/get\s+value\s*\(/.test(src));
  assert.ok(/set\s+value\s*\(/.test(src));
});

test('7. emite eventos de editor (is-persist, is-change, is-delete, is-load)', () => {
  const src = readFileSync(TS, 'utf8');
...
test('8. atributo can-edit bloquea edición', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/['"]can-edit['"]/.test(src) || /readOnly/.test(src), 'can-edit o readonly debe existir');
});

test('9. shadow DOM expone parte label/input/footer', () => {
  const src = readFileSync(TS, 'utf8');
...
test('11. integra con md-render (preview en vivo)', () => {
  const src = readFileSync(TS, 'utf8');
...
test('12. el template expone partes CSS para customización externa', () => {
  const src = readFileSync(TS, 'utf8');
  const partsCount = (src.match(/\bpart\s*=\s*['"][a-z-]+['"]/g) || []).length;
