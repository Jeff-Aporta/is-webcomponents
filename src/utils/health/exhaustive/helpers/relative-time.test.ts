...
test('1. módulo .ts existe y es legible', () => {
...
test('4. OBSERVED incluye date, sync, locale, format', () => {
...
test('5. el módulo reusa parseLooseDate (no reinventa la rueda)', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(
...
test('6. usa Intl.RelativeTimeFormat (no es un polyfill manual)', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/Intl\.RelativeTimeFormat/.test(src), 'debe usar Intl nativo');
});

test('7. tiene un método render() que actualiza el shadow cuando cambia el texto', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/#render\s*\(/.test(src) || /render\s*\(\s*\)/.test(src), 'debe haber un método render()');
});

test('8. atributo sync activa actualización periódica (setInterval)', () => {
  const src = readFileSync(TS, 'utf8');
...
test('9. shadow DOM expone parte formateada (part="text" o similar)', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/<time[^>]*part\s*=/.test(src), 'shadow debe tener <time part=...>');
});

test('10. custom element registrado correctamente', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(
...
test('11. maneja format ∈ {long, short, narrow} (Intl.RelativeTimeFormat)', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/['"]short['"]/.test(src) && /['"]long['"]/.test(src) && /['"]narrow['"]/.test(src),
    'format acepta long/short/narrow');
});

test('12. atributo locale es BCP 47 y delega en resolveLocale', () => {
  const src = readFileSync(TS, 'utf8');
