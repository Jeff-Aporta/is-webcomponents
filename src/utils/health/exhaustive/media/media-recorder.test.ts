...
test('4. OBSERVED incluye source, disabled', () => {
...
test('5. attributeChangedCallback presente (no solo declarativo)', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/attributeChangedCallback\s*\(/.test(src));
});

test('6. usa getUserMedia o getDisplayMedia para captura', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/getUserMedia/.test(src) || /getDisplayMedia/.test(src));
});

test('7. usa MediaRecorder API', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/MediaRecorder/.test(src));
});

test('8. emite is-start, is-stop, is-error', () => {
  const src = readFileSync(TS, 'utf8');
...
test('9. tiene métodos start() y stop() públicos', () => {
  const src = readFileSync(TS, 'utf8');
  assert.ok(/start\s*\(\s*\)/.test(src));
  assert.ok(/stop\s*\(\s*\)/.test(src));
});

test('10. atributo source acepta "camera" | "mic" | "display"', () => {
  const src = readFileSync(TS, 'utf8');
...
test('11. expone parte preview y download', () => {
  const src = readFileSync(TS, 'utf8');
