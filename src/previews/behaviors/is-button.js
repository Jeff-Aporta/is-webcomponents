...
function wirePlayground(root) {
  const btn = root.querySelector('#pgBtn');
  if (!btn) return;

  const colorEl = root.querySelector('#pgColor') || root.querySelector('#pgVariant');
  const variantEl = root.querySelector('#pgColor')
    ? root.querySelector('#pgVariant')
    : (root.querySelector('#pgAppearance') || root.querySelector('#pgVariantAppearance'));
  const textEl = root.querySelector('#pgText');
  const startEl = root.querySelector('#pgStart');
  const endEl = root.querySelector('#pgEnd');
  const pillEl = root.querySelector('#pgPill');
  const caretEl = root.querySelector('#pgCaret');
  const loadingEl = root.querySelector('#pgLoading');
  const disabledEl = root.querySelector('#pgDisabled');
  const hrefEl = root.querySelector('#pgHref');
  const outEl = root.querySelector('#pgOut');
  ...
  const syncIcons = () => {
    for (const el of [...btn.querySelectorAll('is-icon[slot="start"], is-icon[slot="end"]')]) {
      el.remove();
    }
    // Quitar is-icon sin slot (markup viejo en demos).
    for (const el of [...btn.querySelectorAll(':scope > is-icon:not([slot])')]) {
      el.remove();
    }
  ...
  const syncText = () => {
    ...
    const startIcon = btn.querySelector('is-icon[slot="start"]');
  ...
  const syncOut = () => {
    ...
    parts.push('>');
    const start = btn.querySelector('is-icon[slot="start"]');
    const end = btn.querySelector('is-icon[slot="end"]');
    if (start) {
  ...
  const controls = root.querySelector('.playground .controls') || root.querySelector('#playground .controls');
...
function wireEvents(root) {
  const log = root.querySelector('#evtLog');
  if (!log) return;
