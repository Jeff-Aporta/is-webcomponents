...
function applyOffsets(root) {
  const now = Date.now();
  for (const el of root.querySelectorAll('[data-rt-offset]')) {
...
export async function mount(ctx) {
  ...
  const grid = root.querySelector('#localeGrid');
  const caption = root.querySelector('#localeCaption');
  const hints = root.querySelector('#localeHints');
  ...
  if (grid) {
    ...
    for (const tag of supported) {
      ...
      const rt = card.querySelector('is-relative-time');
  ...
  const probe = root.querySelector('#localeProbe');
  const probeOut = root.querySelector('#localeProbeOut');
  const renderProbe = () => {
