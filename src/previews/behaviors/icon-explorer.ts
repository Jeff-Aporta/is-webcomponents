...
export async function mount(ctx: import('../_kit/types.d.ts').PreviewMountContext) {
  ...
  const params = new URLSearchParams(location.search);
      ...
      const isValidSize = (v: number) => Number.isFinite(v) && v > 0 && v <= 4096;
      ...
      const prettyBody = (body: string, indent = '  ') =>
        body.replace(/></g, '>\n<').split('\n').map((l: string) => indent + l.trim()).filter((l: string) => l.trim()).join('\n');
      ...
      async function showIcon(prefix, name: string, list, pos: number) {
        state.prefix = prefix;
        state.name = name;
      ...
      async function renderIndex() {
        ...
        function paintFamilies() {
          ...
          results.querySelectorAll<HTMLElement>('.sample').forEach((el) => io.observe(el));
        }
        ...
        async function paintIcons() {
          ...
          if (!allIcons) {
            ...
            await ensureIcons((done: number, total: number) => {
      ...
      await loadMeta();
      (family ? renderFamily(family) : renderIndex()).catch((err: string) => {
