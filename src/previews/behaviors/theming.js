...
export async function mount(ctx, preview) {
  ...
  function fromOklch({ l, c, h }) {
    ...
    for (let i = 0; i < 24; i += 1) {
      ...
      if ([r, g, b].every((v) => v > 0.0005 && v < 0.9995) || cc < 0.002) break;
      cc *= 0.94;
    }
...
`;
  ...
  const pickers = [...root.querySelectorAll('.tw-seed')];
  const nameInput = root.querySelector('#palName');
  const panels = [...root.querySelectorAll('.tw-panel')];
  const out = root.querySelector('#cssOut');
  const rampEl = root.querySelector('#ramp');
  const tagName = root.querySelector('#tagName');
  const fileName = root.querySelector('#fileName');
  const applyRoot = root.querySelector('#applyRoot');
  const labels = {
    dark: root.querySelector('#darkBgLabel'),
    light: root.querySelector('#lightBgLabel'),
  };

  const tpl = root.querySelector('#tplPanel');
  ...
  function clearRootInline() {
    ...
    for (const k of [...html.style].filter((k) => k.startsWith('--is-'))) {
