 ...
 */
function parallaxCollage(raiz: HTMLElement, signal: AbortSignal) {
  const track = raiz.querySelector<HTMLElement>('#homeCollageTrack');
  if (!track) return;
  const cards = [...track.querySelectorAll<HTMLElement>('.collage-card')];
 ...
 * @param {AbortSignal} signal
 */
function revelarAlEntrar(raiz: HTMLElement, signal: AbortSignal) {
 ...
 * @param {AbortSignal} signal
 */
function contadores(raiz: HTMLElement, signal: AbortSignal) {
  const nodos = raiz.querySelectorAll<HTMLElement>('[data-count-to]');
