...
function tickNow() {
  ...
  for (const el of mainRoot.querySelectorAll('is-format-date')) {
...
export async function mount(ctx) {
  ...
  const grid = mainRoot.querySelector('#localeGrid');
  const caption = mainRoot.querySelector('#localeCaption');
  const hints = mainRoot.querySelector('#localeHints');
  ...
  if (grid) {
    ...
    for (const tag of supported) {
      ...
      card.querySelector('is-format-date').setAttribute('locale', tag);
  ...
  const probe = mainRoot.querySelector('#localeProbe');
  const probeOut = mainRoot.querySelector('#localeProbeOut');
  const renderProbe = () => {
