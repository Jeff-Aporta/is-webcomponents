...
1. `components/LLM.md` define mapa global, reglas obligatorias, reutilización y errores conocidos.
...
- **115 archivos Markdown bajo `components/`** para snapshot base.
...
4. **Asumir cantidad fija de `../` en `script`/`style`.** Prevención: localizar segmento `components/` y derivar ruta física desde ahí.
