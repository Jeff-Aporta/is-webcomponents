...
source: ./bar-chart.js
style: ./bar-chart.css
preview: ../../previews/data-viz/is-bar-chart.html
---
# `<is-bar-chart>`

## Propósito

Wrapper tipado de `<is-chart>` con `type` fijo en `bar`. Misma API
de configuración Chart.js (`config` / `<script type="application/json">`);
el atributo `type` no se cambia.

Este módulo registra `<is-bar-chart>`.

## Cuándo usarlo

Series, distribuciones, relaciones o jerarquías de datos — cuando el tipo
de gráfica es siempre bar chart.

## Cuándo no usarlo

Si el tipo puede cambiar en runtime, usar `<is-chart type="bar">`.
...
| `config` | lectura/escritura | Forma Chart.js; `type` se fuerza a `bar`. |
...
> `<is-bar-chart>` — wrapper tipado vía `defineTypedChart('is-bar-chart', 'bar', …)`.
...
```html
<is-bar-chart label="Bar Chart" legend-position="bottom">
  <script type="application/json">
  {
    "data": {
      "labels": ["Ene", "Feb", "Mar"],
