...
(() => {
  ...
  class IsSpreadsheet extends HTMLElement {
    ...
    constructor() {
      ...
      this.#table = this.shadowRoot.querySelector('.grid');
    ...
    #move(r, c) {
      ...
      const td = this.#table.querySelector(`td.cell[data-r="${r}"][data-c="${c}"]`);
      if (td) td.focus();
    }
  ...
  function move(table, r, c, key) {
    const step = { ArrowUp: [-1, 0], ArrowDown: [1, 0], ArrowLeft: [0, -1], ArrowRight: [0, 1] }[key];
    const nr = r + step[0];
    const nc = c + step[1];
    return table.querySelector(`td.cell[data-r="${Math.max(0, nr)}"][data-c="${Math.max(0, nc)}"]`);
  }
