 ...
 * @returns {string}
 */
export function singleFilterToClause(field, def) {
  if (!def) return '';
 ...
 * @returns {string}
 */
export function convertFilterModelToSQL(filterModel, colIDFields) {
  const clauses = [];
  for (const [colId, def] of Object.entries(filterModel ?? {})) {
    ...
    if (def.filterType === 'multi') {
      const sub = (def.filterModels ?? [])
        .map((sm) => singleFilterToClause(field, sm))
        .filter(Boolean);
 ...
 *
 * Entrada: `{ request: { startRow, endRow, sortModel, filterModel }, idnfiltro }`.
 * ag-Grid pide el rango semiabierto `[startRow, endRow)`; de ahí se derivan
 * `qregistros` (tamaño de página) y `pagina` (1-based).
 *
 * @param {{request: {startRow?: number, endRow?: number, sortModel?: any[], filterModel?: any}, idnfiltro?: string, colIDFields?: Map<string,string>}} params
 * @returns {Object} TFiltroLista
 */
 ...
 * éxito reporta `{ rowData, rowCount }`, en el error llama `params.fail()`.
 *
 * @param {{Lista: (f:any)=>Promise<any>, idnfiltro?: () => string, colIDFields?: Map<string,string>, onLoad?: (lista:any)=>void}} opts
 * @returns {{getRows: (params:any)=>Promise<void>}}
 */
 ...
 */
function evalClause(clause, row) {
  let c = clause.trim();
 ...
 */
export async function createFakeListaFromUrl(url, opts = {}) {
